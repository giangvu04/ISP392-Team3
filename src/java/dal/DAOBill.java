...
public class DAOBill {
    ...
    public ArrayList<Bill> getBillsByPage(Users user, int page, int billsPerPage) {
        ...
        int roleID = user.getRoleId();
        int tenantID = user.getUserId(); // manager thì sẽ load hết bill còn là tenant thì chỉ load của họ
        String sql = (roleID != 3) ? 
            "SELECT * FROM dbo.tbBills b \n" +
        "JOIN dbo.rooms r ON r.room_id = b.RoomID\n" +
"JOIN dbo.rental_areas ra ON ra.rental_area_id = r.rental_area_id\n" +
"WHERE ra.manager_id = ?\n" +
"ORDER BY CreatedDate DESC OFFSET ? ROWS FETCH NEXT ? ROWS ONLY" : 
            "SELECT * FROM tbBills WHERE TenantID = ? ORDER BY CreatedDate DESC OFFSET ? ROWS FETCH NEXT ? ROWS ONLY";
